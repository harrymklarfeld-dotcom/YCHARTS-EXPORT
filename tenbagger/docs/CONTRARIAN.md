...
## 2026-09-26: Personalized budgeting system (onboarding, budget engine, Insights engine)

**What was proposed:** an onboarding flow (goals, income sources including hourly and per-gig pay, fixed bills, spending categories, and a choice of budgeting style: paycheck-to-paycheck as the default, 50/30/20, zero-based, pay-yourself-first). Behind it sit a budget engine (safe-to-spend, per-paycheck allocation, envelopes, goals, buffer month) that works without bank linking, and an "Insights" engine (subscription detector, card payoff planner, pending-pay reminders, company-metric analogies that link to lessons).

**Status check since yesterday:** `discovery/tracker.csv` still has only the example row. Interviews are still at zero, and this proposal adds a fourth product surface to the three that yesterday's review said to unbundle.

**Steelman.** Students with irregular pay are badly served: the forecasting apps cost $95–$199 a year and assume a salary, and the apps aimed at irregular earners make their money from advances that the FTC has pursued (MONEY_HUB §1). A no-link, paycheck-to-paycheck planner that counts pay you've earned but not yet received, framed as "run your money like a company", is a real gap and a natural bridge to the lessons.

**Attack.**
- **It is a whole budgeting app, built from scratch, against incumbents that are free to students.** YNAB and Monarch both give .edu students 12 months free (MARKET_ENTRY §3). Rocket Money and PocketGuard have free tiers with "left to spend" and subscription tracking (MONEY_HUB §1). EveryDollar already sells zero-based paycheck planning with free manual entry. Four budgeting styles means four engines to build and test, each competing with an app that does only that one style. MARKET_ENTRY §4 already lists "budgeting dashboards" as **not defensible**, and its Monarch row says "don't fight on budgeting".
- **Budgeting apps churn, and manual ones churn fastest.** Mint had tens of millions of users and still shut down in March 2024 because it could not make free budgeting pay ([CNBC](https://www.cnbc.com/2023/11/07/budgeting-app-mint-is-shutting-down-users-are-disappointed.html), [WalletHub](https://wallethub.com/edu/b/what-happened-to-mint/151868)). Finance-app day-30 retention benchmarks range from about 3% to 12% depending on the source ([UXCam](https://uxcam.com/blog/mobile-app-retention-benchmarks/), [Plotline](https://www.plotline.so/blog/retention-rates-mobile-apps-by-industry)) [S/U]. Vendor blogs put budgeting-app abandonment at about 60–70% within 30 days and name manual logging as the top reason ([SpendTrak](https://spendtrak.app/blog/why-people-quit-budgeting-apps), [Strategia-X](https://www.strategia-x.com/blog/2026-04-12-why-budgeting-apps-fail-30-days-fintech-ux-data/)) [U: the survey figures these blogs cite are unverified]. Envelopes plus category tracking without a bank link means the student types in every coffee, which is exactly the effort that kills these apps.
- **The Insights engine contradicts the "no bank linking" promise.** A subscription detector needs transaction history. Without Plaid it can only list subscriptions the user typed in, which is a list, not a detector. So one of two things happens. Either the Insights feature is hollow, or it quietly pulls Plaid back in, along with the per-Item fees (16% of FY3 revenue) and the GLBA duties that MARKET_ENTRY D4 deferred.
- **The onboarding length alone will lose users.** Goals, income sources with hourly rates, bills, categories and a style choice come to about 15–25 inputs before the first bit of value. The onboarding ask is the product's biggest activation risk, and it stacks a second setup flow on top of the lesson onboarding. A "safe-to-spend" figure built on guessed inputs is wrong from day one. That brings back the Hello Digit risk: a confident green number that causes an overdraft.
- **It is the opposite of the focus decision, and it creates another place to cross the advice line.** A "card payoff planner" that ranks which debt to pay first, or "pay-yourself-first → invest" nudges, sit close to individualised recommendations. The build effort is L to XL, and it buys none of the PRODUCT_STRATEGY MVP (Guess the Company, Quests, the Interview track), none of which exists yet.

**Reframe.**
- **Cheap test (1 week), a spreadsheet concierge:** a Google Sheet template called "Paycheck Plan" (pay in, bills, card due date, hours not yet paid, safe-to-spend) plus a Sunday text asking "update your 3 numbers". Recruit 15 working students through the club and campus job boards. Pass bar: at least 5 of them still update it in week 2 without being chased. If fewer do, no budget engine will fix that. This costs zero code and doubles as interview recruiting.
- **10x weirder, "Your money's 10-Q":** skip the budget entirely. Once a month the student answers 4 questions (money in, money out, card balance, pay not yet received) and gets back a one-page quarterly-report card of their own finances: revenue, burn, runway in days, a "going-concern" flag, and a link to the lesson that explains each line. It is shareable (with numbers hidden), it teaches reading real filings, it is used monthly rather than daily, and nobody else does it. It is the company-metric analogy as the whole product, not an Insights side-panel.
- **Do less, one number:** build only **safe-to-spend until the next paycheck, for the paycheck-to-paycheck case**, using 4 inputs (cash now, next pay date and estimated amount, bills due before then, pay not yet received). No styles, no envelopes, no categories, no detector. It is essentially the Due-Date Check that MARKET_ENTRY D4 already chose, and the other styles become lessons ("what is 50/30/20?") rather than engines.
- **Partner instead of build:** since YNAB and Monarch are free to students for a year, write a lesson unit, "Budget like a CFO", that teaches the four styles and hands off to them. You get the education without the churn or the COGS.

**Verdict: change (close to kill for the full scope).** Kill the four-style engine, the envelopes and the Insights engine for v1. The single change that helps most is to **shrink it to one manual "safe-to-spend until payday" number with 4 inputs**, behind the D4 gate: run the spreadsheet concierge first, and build it in-app only if at least 30% of testers update it weekly. Teach the budgeting styles as lessons instead of building them as engines.
